...
#Building the tune, but for the 7-day horizon we only load the parameters from the 24 hour horizon
def build_and_tune_GRU(X_train, y_train, X_val, y_val, n_timesteps, n_features, horizon, dataset_name):
    horizon_str = f"{horizon}H"
    json_file = f'best_GRU_params_{horizon_str}_{dataset_name}.json'
    with open(json_file, 'r') as f:
        best_params = json.load(f)
    print("Loaded best GRU parameters:", best_params)
    return best_params
...
def build_final_model(best_params, X_train, y_train, X_val, y_val, n_timesteps, n_features, horizon, dataset_name, datetime_val = None, regions_val = None ):
    ...
    tracker = CarbonTracker(epochs=15, components='gpu', log_dir=carbon_log_dir)
    ...
    final_model.fit(
        ...
        epochs=15, # epoch setting how many times trained
...
def main():
    ...
    lookback = 336   # lookback of 168 means, looking back 2 weeks per row
    horizon = 168   # horizon of 168, predicting 1 week ahead
    group_col = 'region_id'
