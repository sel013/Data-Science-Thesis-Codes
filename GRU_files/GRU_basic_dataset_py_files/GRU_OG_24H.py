...
def load_and_preprocess_data(dataset_path='Hourly_Elec_Original_Data.pkl'):
...
def GRU_tuning(trial, X_train, y_train, X_val, y_val, n_timesteps, n_features, horizon):
    ...
    activation = 'tanh' # standard activation function for GRU

    #intializing the models
    model = Sequential()
    for i in range(n_layers):
        return_seq = (i < n_layers - 1)
        #adding the GRU layer for the model.
        model.add(GRU(
            n_units,
...
def build_and_tune_GRU(X_train, y_train, X_val, y_val, n_timesteps, n_features, horizon, dataset_name):
    horizon_str = f"{horizon}H"
    study_name = f"GRU_forecast_{horizon_str}_{dataset_name}"
    storage = f"sqlite:///optuna_GRU_forecast_{horizon_str}_{dataset_name}.db"
    ...
    study.optimize(lambda trial: GRU_tuning(trial, X_train, y_train, X_val, y_val, n_timesteps, n_features, horizon),
...
def make_val_subset_per_month(X_val, y_val, datetime_val, regions_val, week_hours=24*7):
    keep_idx = [] #list for storage of rows that will be kept

    #looping thorugh each region
    for reg in np.unique(regions_val):
        reg_mask = regions_val == reg   # creating the masks to only get rows that belong to the specific region
        reg_idx = np.where(reg_mask)[0] # getting the row indices of this region of the full dataset
        reg_datetimes = datetime_val[reg_mask]  # extracting the timestamps that belong to this region row
    ...
    #Creating the subset data for the model training validation
    X_sub = X_val[keep_idx]
    y_sub = y_val[keep_idx]
...
def build_final_model(best_params, X_train, y_train, X_val, y_val, n_timesteps, n_features, horizon, dataset_name, datetime_val = None, regions_val = None ):
    horizon_str = f"{horizon}H"
    #initializing the model
    final_model = Sequential()
     #Adding the GRU layers and using the best parameters from the optuna results
    ...
    #downsizing the validation dataset by using the defined function of selection 1 week to keep the model computational managable on the GPU
    X_val_sub, y_val_sub = make_val_subset_per_month(
        X_val, y_val, datetime_val, regions_val, week_hours=24*7
    )
    ...
    tracker = CarbonTracker(epochs=20, components='gpu', log_dir=carbon_log_dir)
    ...
    final_model.fit(
        ...
        epochs=20, # epoch setting how many times trained
...
def plot_permutation_importance_top(model, X_test, y_test, feature_names, horizon=1,
    ...
    plot_file = f'GRU_{horizon}H_{dataset_name}_feature_importance.png'
    csv_file = f'GRU_{horizon}H_{dataset_name}_feature_importance_table.csv'
...
def main():
    ...
    lookback = 168   # lookback of 168 means, looking back 7 days per row
    horizon = 24   # horizon of 24, predicting 24 hour ahead
    group_col = 'region_id'
    ...
    X_train, y_train, X_val, y_val, X_test, y_test, regions_test, datetime_test, test_mask, val_mask = split_data(X, y, datetime_target, regions_seq)

    n_timesteps = X_train.shape[1]
    n_features = X_train.shape[2]

    best_params = build_and_tune_GRU(X_train, y_train, X_val, y_val, n_timesteps, n_features, horizon, dataset_name)
    final_model = build_final_model(best_params, X_train, y_train, X_val, y_val, n_timesteps, n_features, horizon, dataset_name, datetime_val = datetime_target[val_mask], regions_val = regions_seq[val_mask])
    ...
    eval_df = pd.DataFrame({
        'Model_name': f'GRU_OG_{horizon}H',
    ...
    csv_file = f'GPU_Predictions_GRU_{horizon}H_{dataset_name}.csv'
